## Reprobuild dev env + build recipe for codetracer-circom-recorder.
##
...
## sibling recorders (evm, cairo, …) model the identical dependency:
## the toolchain floor for the Nim FFI that
## ``codetracer_trace_writer_nim``'s ``build.rs`` compiles at cargo
## build time (``nim`` + ``nimble`` + ``capnp`` + ``zstd``) is declared
## in ``uses:``, and cargo does the cross-crate wiring itself.
##
...
## …)]`` / ``#[ignore]`` selection (they compile ``.circom`` fixtures
## via the pinned ``circom`` compiler and assert on the produced CTFS
## bundle on every host cargo + circom support), so the single
...
## ``rustc`` / ``nim`` / ``nimble`` / ``capnp`` / ``circom`` / ``zstd``
## on ``PATH`` (and ``PKG_CONFIG_PATH`` for libzstd + openssl), so the
## weak-local PATH resolver is the right default. Without it ``repro
## build`` refuses to run with "typed tool provisioning is required for
## uses declarations".
##
