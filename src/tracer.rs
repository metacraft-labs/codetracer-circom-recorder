...
impl CircomTracer {
    ...
    fn start_trace(source_path: &Path, out_dir: &Path) -> Result<Self> {
        ...
        // ``start`` opens the depth-0 ``<toplevel>`` frame itself, which is
        // what the comments throughout this recorder and the EVM recorder
        // already assumed.  The depth-tracking semantics documented in commit
        // 9f4bb20 are unchanged: the main template's steps remain at depth 0
        // inside that frame (its Call is intentionally skipped, see
        // ``emit_template_fns``), sub-components nest at depth 1+, and the
        // closing ``register_return`` in write_trace closes it back out.
