...
- Circom has no native stdout / stderr. The closest analogues are the
  ...
  errors which fail the witness pipeline. The recorder now routes compiler /
  witness failures and WASM-backend `log()` output as special events; only
  assert-specific classification remains open.
...
| b   | `register_call` for each call (template instantiation)                                    | OK                                                       | OK                                                   | The recorder registers function metadata for each parsed template, then emits `register_call` / `register_return` for each concrete non-root component instantiation (`component adder = Adder()`). The root `component main` is intentionally merged into the `<toplevel>` call that `TraceWriter::start` opens, so its body lives at depth 0 — this design pre-dates the audit and is documented in `tracer.rs::emit_source_trace`. Per the "circom is not a stack VM" note above, the unit being treated as a "call" is component/template instantiation, not a runtime function call with argument values.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| c   | Call args via `register_call_arg` / `arg()`                                               | **GAP**                                                  | **OK**                                               | Pre-fix the call-detection branch always called `register_call(fn_id, vec![])` — every Circom template invocation showed empty arguments in the calltrace pane even when the template had a non-empty `signal input` list. The 1.58 fix extended `parse_template_definitions` to collect each template's `signal input` declarations into `TemplateDef.input_signals: Vec<String>` and staged the names via `TraceWriter::arg(input_name, NONE_VALUE)`. The 2026-05-04 follow-up closes live per-instance binding for concrete component calls: `parse_component_instances` captures `component <inst> = <Template>(...)` declarations, `emit_source_trace` registers calls from those instantiation sites, and each declared input is staged with `ValueRecord::Int` from the witness `.sym` map key `<inst>.<input_name>` when available, falling back to `NONE_VALUE` only if the compiler omits that signal. The read-side regression opens the `.ct` with `NimTraceReaderHandle` and asserts `Adder`'s `CallRecord.args` carries typed integer values for `a` and `b`. Non-zero user-supplied main inputs remain a separate CLI/API integration boundary because the public recorder path currently hardcodes generated witness input JSON to zeros. |
| d   | Write/WriteOther/Error/EvmEvent for IO and structured events via `register_special_event` | **N/A (Write)** / **OPEN (Error)** / **OPEN (EvmEvent)** | **N/A (Write)** / **OK (Error)** / **OK (EvmEvent)** | Circom has no stdin/stdout-style IO, so Write remains N/A. Error routing is closed for compiler / WASM / wasmtime / C++ witness failures via `register_special_event(EventLogKind::Error, ...)`. EvmEvent routing is closed for WASM-backend `log()` directive output: the Wasmtime host callbacks now mirror the generated `witness_calculator.js` buffering protocol and route each flushed message through `register_special_event(EventLogKind::EvmEvent, "circom_log", msg)`. The CTFS reader currently collapses EvmEvent into the `stderr` IO bucket, so readback tests assert that bucket plus message content.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
At that point the EvmEvent half of row (d), `log()` directive routing through
`"circom_log"`, still remained open; the next follow-up closed it.

2026-05-03 follow-up: audit row (d)'s EvmEvent portion is now closed for
the WASM witness backend. The Wasmtime runtime callbacks now mirror the
generated `witness_calculator.js` buffering protocol: `writeBufferMessage`
collects string chunks, `showSharedRWMemory` appends the current field value,
and a newline flushes one Circom `log()` message. Each flushed message routes
through `register_special_event(EventLogKind::EvmEvent, "circom_log", msg)`.
The CTFS reader collapses EvmEvent into the `stderr` IO bucket, so the focused
readback test asserts that bucket plus the logged message content.
...
2026-05-04 follow-up: the live per-instance binding layer is now wired for
concrete component instantiations. `parse_component_instances` captures each
`component <inst> = <Template>(...)` declaration, excluding the root
`component main` frame that is already represented by `<toplevel>`. At trace
emission time the recorder registers calls from those instantiation sites,
looks up the callee template's `input_signals`, and stages each argument with
the witness value stored under `<inst>.<input_name>` in the `.sym`-derived
`values` map. If a compiler optimization omits a signal, the fallback remains
`NONE_VALUE` rather than aborting trace emission. The public recorder API still
hardcodes generated witness input JSON to zero, so non-zero user-supplied main
inputs need a separate CLI/API input-file layer.

## Tests added

`tests/test_ctfs_audit.rs` (5 cases after the 2026-05-03 follow-ups):
...
- `call_arg_staging_records_live_component_input_values` — writes a
  small `Adder` circuit, records it, opens the `.ct` via
  `NimTraceReaderHandle`, and asserts a call record contains args
  `a` and `b` decoded as typed `ValueRecord::Int` witness values.
...
- `circom_log_directive_emits_evm_event_special_event` — writes a circuit
  that executes `log("input", in, "out", out)`, records it with the WASM
  backend, opens the `.ct` via `NimTraceReaderHandle`, and asserts the
  EvmEvent-routed log appears in the collapsed `stderr` bucket with the
...
special event, the `log()` EvmEvent route, and live component input call args
via the existing `codetracer_trace_writer_nim` `NimTraceReaderHandle`.
...
- lib unit tests: 40 / 40 passing (was 37; the audit adds
  `test_parse_template_definitions_collects_input_signals` and the
  2026-05-04 follow-up adds `test_parse_component_instances`; the
...
- `test_ctfs_audit`: 5 / 5 passing.

Total: 58 / 58 passing across all suites, 0 regressions.
...
### Per-instance input-signal value resolution (audit c, live values) — CLOSED 2026-05-04

The recorder now has the mapping layer described above: concrete component
instances are parsed, matched to their template definitions, and staged with
`ValueRecord::Int { i, type_id }` values from the witness map before the
sub-template `register_call`. The new read-side assertion covers the embedded
`CallRecord.args` content, not just container shape.

Remaining adjacent boundary: the public recorder path synthesizes witness
input JSON internally and defaults every main input to `0`. Non-zero
user-supplied main input values need a separate CLI/API input-file layer; this
is not a blocker for the per-instance binding path, which now stages whatever
values the witness run produced.
...
### `log()` directive routing (audit d, EvmEvent) — CLOSED 2026-05-03
...
`console_log`, Cairo's `Print`, Cadence's `log()`.

The recorder now captures the WASM-side output by replacing the no-op
`writeBufferMessage` / `showSharedRWMemory` callbacks with closures that mirror
the generated JavaScript witness calculator's buffering protocol and route each
flushed log line through
`register_special_event(EventLogKind::EvmEvent, "circom_log", msg)`. Routing
to `EvmEvent` rather than `Write` follows the EVM 1.39 / Cairo 1.50 / Fuel
1.53 / PolkaVM 1.55 / Miden 1.56 pattern for structured events from
compute-only VMs.
...
The compile-error special-event route, `log()` EvmEvent route, and live
component input call args are covered by read-side assertions via
`NimTraceReaderHandle`.
...
arg-name staging plus live per-instance witness-value binding via
`TraceWriter::arg(name, ValueRecord::Int)` + compile / WASM / wasmtime /
C++ witness Error special-event routing + `log()` EvmEvent routing;
assert-specific classification + per-WASM-instruction step emission +
`.wtns` replay path + user-supplied witness input JSON remain open as
recorder-side / parser / writer-API / CLI-API follow-ups). Audited recorder
count: 14 → 15.
...
Circom recorder on 2026-05-08, mirroring the Cairo recorder
2026-05-08 follow-up landed in commit `2710b5e` of
`codetracer-cairo-recorder` and the Cardano recorder 2026-05-08
follow-up landed in commit `0698f00` of `codetracer-cardano-recorder`:
...
- The JSON / legacy-binary output paths were removed. The recorder's
  ...
  site: `tracer.rs::CircomTracer::trace_program` (and
  `trace_program_cpp`, `trace_program_with_backend`,
  `trace_program_no_srcmap_with_tracer`) and `recorder.rs::record` no
  longer take a `format` parameter. The `events_filename` match in
  `tracer.rs::start_trace` (which used to dispatch on
  `Json` / `Binary` / `BinaryV0` / `Ctfs`) was collapsed to a single
  `trace.bin` arm — the underlying writer materialises a `.ct`
  multi-stream container regardless of the path basename.
- `CODETRACER_CIRCOM_RECORDER_OUT_DIR` was added as a fallback for
  `--out-dir`. Lookup order is CLI flag → env var → `./ct-traces/`.
- `CODETRACER_CIRCOM_RECORDER_DISABLED=1` (or `true`) skips trace
  emission entirely; the Circom recorder doesn't run a separate target
...
- Tests in `tests/test_tracer.rs` that previously drove the CLI with
  `--format ctfs` were rewritten to drive the recorder library
  directly and assert on the produced `.ct` container. A new
  `test_recorded_trace_via_ct_print_json` records `flow_test.circom`
  and pipes the resulting `.ct` container through `ct-print --json`.
  The Circom recorder emits its variable values through
  `register_variable_with_full_value` with `ValueRecord::Int`
  payloads, but the integer values do not round-trip through
  `ct-print --json` today (same pre-existing limitation as the cardano
  recorder's "Variable types are always Int" follow-up); the test
  therefore asserts on **structural anchors** that the recorder must
  surface for any CodeTracer consumer to function — the source path,
  the `FlowTest` template name, and each declared signal name (`a`,
  `b`, `sum_val`, `doubled`, `out`). The test skips gracefully (with a
  printed `SKIP:` line) when `ct-print` is not present (e.g. when the
...
- `tests/test_ctfs_audit.rs` was updated to drop the
  `TraceEventsFileFormat` parameter from each `recorder::record(...)`
  call site, and the old `ctfs_format_advertised_in_record_help` test
  (which would have locked in the regression by asserting `--format`
  / `[default: ctfs]` were present in `record --help`) was replaced
  with the post-fix `test_no_format_flag_in_help` and
  `test_help_mentions_ct_print` tests.
- `tests/verify-cli-convention-no-silent-skip.sh` was added as a
  ...
  mentioned in the top-level `--help`, and asserts that
  `CODETRACER_CIRCOM_RECORDER_OUT_DIR` and
  `CODETRACER_CIRCOM_RECORDER_DISABLED` are referenced in source.
...
- [`codetracer-cairo-recorder` commit `2710b5e`](../codetracer-cairo-recorder/) — the Cairo recorder follow-up.
- [`codetracer-cardano-recorder` commit `0698f00`](../codetracer-cardano-recorder/) — the Cardano recorder follow-up that this work mirrors most closely (Circom shares Cardano's `ValueRecord::Int` round-trip limitation).
