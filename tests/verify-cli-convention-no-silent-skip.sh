#!/usr/bin/env bash
# Verify that the codetracer-circom-recorder CLI complies with
...
#   * `CODETRACER_CIRCOM_RECORDER_OUT_DIR` is referenced in source so the
#     env-var fallback (§5) cannot regress silently.
#   * `CODETRACER_CIRCOM_RECORDER_DISABLED` is referenced in source.
#
...
assert_present "codetracer-circom-recorder" "--version output" "${VERSION_OUT}"
...
echo "ok: CODETRACER_CIRCOM_RECORDER_DISABLED referenced in src/"
